...
public class AutoBlue_matt extends OpMode {
...
//      sets the starting positions of the servos
        ...
        LeftPosition = backLeft.getCurrentPosition();
        RightPosition = backRight.getCurrentPosition();

//      sets the direction of the wheels
        backLeft.setTargetPosition((int) leftCount + backLeft.getCurrentPosition());
        backRight.setTargetPosition((int) rightCount + backRight.getCurrentPosition());
        frontLeft.setTargetPosition((int) leftCount + frontLeft.getCurrentPosition());
        frontRight.setTargetPosition((int) rightCount + frontRight.getCurrentPosition());
...
//      makes it so the program can advance the switch statement after moving
        if  ((Math.abs(backLeft.getCurrentPosition())/(0.01+Math.abs(backLeft.getTargetPosition()))*100 > 95)&&
                (Math.abs(backRight.getCurrentPosition())/(0.01+Math.abs(backRight.getTargetPosition()))*100 > 95)) {
